...
internal class Method {
    ...
    constructor(kotlinFunction: KFunction<*>, isLambda: Boolean) {
        ...
        this.returnParameter = Parameter(returnType, kotlinFunction.name, false)
    ...
    constructor(javaMethod: JavaMethod, genericArguments: List<KTypeProjection>, isLambda: Boolean) {
        ...
        this.returnParameter = outParameterType?.let { Parameter(it, this.name, true) }
            ?: Parameter(Unit.javaClass, this.name)

        this.parameters = inParameterTypes.map { Parameter(it, this.name, true) }
            .toTypedArray()
    }
